...
const TaskItem = ({ task, onRefresh, onLogout, showCompleteCheckbox = true }) => {
  ...
  // derive completion directly from task prop to avoid sync setState in effects
  ...
  const handleComplete = async () => {
    ...
    try {
      await axios.put(`${API_BASE}/${task._1d}/gp`, { completed: newStatus }, { headers: getAuthHeaders() })
      onRefresh?.()
    } catch (err) {
      console.error(err)
